...
const CORE_MODULES = [
    ...
    { id: 'body', name: 'Body', icon: 'bi-person-arms-up', frequency: { rule: 'all', days: [] }, practices: ['Yoga', 'Cardio', 'Nutrition'] },
    { id: 'mind', name: 'Mind', icon: 'bi-lightbulb', frequency: { rule: 'all', days: [] }, practices: ['Study', 'Concentration', 'Journaling'] },
    { id: 'spirit', name: 'Spirit', icon: 'bi-stars', frequency: { rule: 'all', days: [] }, practices: ['Meditation', 'Contemplation'] },
    { id: 'shadow', name: 'Shadow', icon: 'bi-eye', frequency: { rule: 'all', days: [] }, practices: ['Inquiry', 'Therapy'] },
];
...
function loadConfig() {
    ...
    if (storedConfig) {
        try {
            ...
            // FIX: Also migrate broken "mojibake" icons to Bootstrap Icons if detected
            const normalizedConfig = parsedConfig.map(m => {
                let icon = m.icon;
                // Replace specific known bad strings or generic check
                if (m.id === 'body' && !icon.startsWith('bi-')) icon = 'bi-person-arms-up';
                if (m.id === 'mind' && !icon.startsWith('bi-')) icon = 'bi-lightbulb';
                if (m.id === 'spirit' && !icon.startsWith('bi-')) icon = 'bi-stars';
                if (m.id === 'shadow' && !icon.startsWith('bi-')) icon = 'bi-eye';
...
function renderHeader() {
    ...
    h1.innerHTML = `
                <i class="bi bi-stack me-2 fs-3 text-gradient-primary"></i> 
    ...
    card.innerHTML = `
                <div class="card-header ${headerBg} d-flex justify-content-between align-items-center border-bottom-0 pt-4 pb-2 px-4 bg-transparent">
                    <h5 class="card-title mb-0 d-flex align-items-center">
                        <i class="me-3 fs-4 ${iconColorClass} ${module.icon}"></i>
        ...
        moduleDiv.innerHTML = `
                    <div class="card-header bg-transparent d-flex justify-content-between align-items-center py-3 border-bottom-0">
                        <h4 class="h5 fw-bold mb-0 d-flex align-items-center text-body">
                            <i class="me-3 fs-4 accent-${module.id} ${module.icon}"></i> 
                            ${module.name}
                        </h4>
    ...
    state.config.forEach(module => {
        ...
        moduleDiv.innerHTML = `
                    <div class="d-flex align-items-center gap-3 mb-3">
                        <i class="fs-4 lh-1 accent-${module.id} ${module.icon}"></i>
